...
import { BOARDING_PASS_DEMO_ORDER } from '../data/boardingPassDemo';
import '../styles/Invitation.css';

const API = import.meta.env.VITE_API_URL || 'http://localhost:4000/api';

export default function Invitation({ demo = false }) {
  ...
  const [rsvpForm, setRsvpForm] = useState({ guestName: '', attending: 'yes', guestCount: 1, mealPreference: 'standard', message: '' });
  const [rsvpSubmitted, setRsvpSubmitted] = useState(false);
  const [rsvpError, setRsvpError] = useState('');

  useEffect(() => {
    if (demo) {
      setOrder(BOARDING_PASS_DEMO_ORDER);
      setLoading(false);
      return;
    }
  ...
  if (loading) return <div className="inv-loading"><div className="inv-spinner" /><p>Loading your invitation...</p></div>;
  if (error) return <div className="inv-error"><h2>Invitation Not Found</h2><p>{error}</p></div>;
  ...
  // ===================== SPLASH SCREEN (reference: Framer Motion) =====================
  if (showSplash) {
    ...
    <div className="inv-page boarding-pass-theme">
      ...
      {!isReferenceDemo && couplePhotos.length > 0 && (
        <section className="inv-section-light">
          <h2 className="inv-section-label text-gold">CAPTAINS</h2>
          ...
          <div className="inv-story-timeline">
            ...
            {storyPhotos.map((photo, i) => (
              <motion.div
                ...
                transition={{ duration: 0.6, delay: 0.1 }}
              >
                <div className="inv-story-content" />
                <div className="inv-story-dot">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M17.8 19.2L16 11l3.5-3.5C21 6 21.5 4 21 3c-1-.5-3 0-4.5 1.5L13 8 4.8 6.2c-.5-.1-.9.1-1.1.5l-.3.5c-.2.4-.1.9.3 1.1l5.7 3.3-3.3 3.3-2-.7c-.4-.1-.8 0-1 .3l-.2.3c-.2.3-.1.7.1.9l3.3 2.3 2.3 3.3c.2.3.6.3.9.1l.3-.2c.3-.2.4-.6.3-1l-.7-2 3.3-3.3 3.3 5.7c.2.4.7.5 1.1.3l.5-.3c.4-.2.6-.6.5-1.1z"/></svg>
                </div>
                <div className="inv-story-photo">
      ...
      <section className="inv-section-light">
        <motion.div
          ...
          transition={{ duration: 0.6 }}
        >
          <h2 className="inv-section-label text-gold">BOARDING PASS</h2>
          <h3 className="inv-rsvp-title">CONFIRM YOUR SEAT</h3>

          <AnimatePresence mode="wait">
            {!rsvpSubmitted ? (
              <motion.div key="form" className="inv-rsvp-wrap" exit={{ opacity: 0, y: -20 }}>
                <form onSubmit={handleRsvp} className="boarding-pass-card inv-rsvp-card">
              <div className="inv-form-field">
                <label className="data-label">PASSENGER NAME</label>
                <input type="text" required className="kiosk-input" value={rsvpForm.guestName} onChange={e => setRsvpForm({ ...rsvpForm, guestName: e.target.value })} placeholder="..." />
              </div>
              <div className="inv-form-field">
                <label className="data-label">NUMBER OF PASSENGERS</label>
                <input type="number" min={1} max={10} className="kiosk-input" style={{ width: '96px' }} value={rsvpForm.guestCount} onChange={e => setRsvpForm({ ...rsvpForm, guestCount: parseInt(e.target.value) || 1 })} />
              </div>
              <div className="inv-form-field">
                <label className="data-label">WILL YOU BE BOARDING?</label>
                <div className="inv-radio-group">
                  <button type="button" className={`inv-radio-btn ${rsvpForm.attending === 'yes' ? 'active' : ''}`} onClick={() => setRsvpForm({ ...rsvpForm, attending: 'yes' })}>YES</button>
                  <button type="button" className={`inv-radio-btn ${rsvpForm.attending === 'no' ? 'active' : ''}`} onClick={() => setRsvpForm({ ...rsvpForm, attending: 'no' })}>NO</button>
                </div>
              </div>
              {rsvpForm.attending === 'yes' && (
                <div className="inv-form-field">
                  <label className="data-label">MEAL PREFERENCE</label>
                  <select className="kiosk-input" value={rsvpForm.mealPreference} onChange={e => setRsvpForm({ ...rsvpForm, mealPreference: e.target.value })}>
              ...
              <div className="inv-form-field">
                <label className="data-label">MESSAGE TO CREW</label>
                <textarea className="kiosk-input" rows={3} value={rsvpForm.message} onChange={e => setRsvpForm({ ...rsvpForm, message: e.target.value })} placeholder="..." />
              </div>
              {rsvpError && <p className="inv-rsvp-error">{rsvpError}</p>}
              <button type="submit" className="inv-rsvp-submit">CHECK IN</button>
                </form>
              </motion.div>
            ) : (
              <motion.div
              ...
              >
                <div className="boarding-pass-card inv-rsvp-success-card">
                  <motion.div
      ...
      <section className="inv-footer">
        <div className="perforation-horizontal" />
        <motion.div
        ...
        >
          <div className="inv-monogram">
            <span className="inv-mono-star">✦</span>
            <span className="inv-mono-initials">{initials}</span>
            <span className="inv-mono-star">✦</span>
          </div>
          <p className="inv-footer-thanks">Thank you for flying with us</p>
          <p className="inv-footer-couple text-gold">{name1} & {name2}</p>
